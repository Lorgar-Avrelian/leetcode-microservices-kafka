...
@Service
public class Task0001RunService implements TaskRunService<Input, int[]> {
    private static final TestValues<Input, int[]> testValues = new TestValues<>(
            Map.of(
                    new Input(new int[]{2, 7, 11, 15}, 9), new int[]{0, 1},
                    new Input(new int[]{3, 2, 4}, 6), new int[]{1, 2},
                    new Input(new int[]{3, 3}, 6), new int[]{0, 1}
            )
    );
    @Value("${task.number}")
    ...
    @Override
    public TaskReport<Input, int[]> runTask() {
        Task0001 t = new Task0001(testValues);
        TaskRunUtil<Input, int[]> taskRunUtil = new TaskRunUtil<>(t);
        List<RunResult<Input, int[]>> runResults = taskRunUtil.doTask(testValues);
        TaskReport<Input, int[]> report = new TaskReport<>();
